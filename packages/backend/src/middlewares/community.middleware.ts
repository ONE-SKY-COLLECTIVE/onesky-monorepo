...
// Middleware to validate update community
export const validateUpdateCommunity = [
  param('communityId').isUUID().withMessage('Invalid community ID format.'),
  body('name')
    .optional()
    .trim()
    .isLength({ min: 6, max: 255 })
    .withMessage('Community name must be between 6 and 255 characters.'),
  body('description')
    .optional()
    .trim()
    .isLength({ max: 1000 })
