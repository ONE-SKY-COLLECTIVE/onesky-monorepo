...
const ConfirmExit: React.FC<{ utility: () => void }> = ({ utility }) => {
  ...
  return (
    <View className="flex-col absolute bottom-0 px-7 pt-10 pb-20 items-center self-center w-[100vw] bg-[#FCF2E2] rounded-t-[36px]">
      ...
      <TouchableOpacity
        className="w-full bg-[#FFFFFF] py-[10px] rounded-[8px] mb-2 mt-3 border-[1.3px] border-[#C7E28C]"
        onPress={() => router.replace('/(tabs)/(home)')}
      >
        <Text className="text-center">Back to Home Page</Text>
      </TouchableOpacity>
      <TouchableOpacity
        className="w-full bg-[#A1CE3F] py-[10px] rounded-[8px] mt-2"
        onPress={utility}
      >
