...
export default function ExpoForm() {
  ...
  return (
    <SafeAreaView edges={['top']} className="flex-1 justify-between bg-[#C4EFF7]">
      ...
      <View className="z-20 rounded-t-3xl bg-white p-4 pb-12" style={{ marginTop: -400 }}>
        ...
        {formFields.map(({ key, label, placeholder, icon, secure }, index) => (
          <View key={index} className="mb-4">
            ...
            >
              <Image source={icon} alt="An icon" style={{ width: 16, height: 16 }} />
              <Controller
                control={control}
            ...
            {errors[key as keyof FormData] && (
              <Text className="text-xs text-red-500">{errors[key as keyof FormData]?.message}</Text>
            )}
          </View>
        ))}
        <Link className="mb-3 text-right text-sm underline" href={'/(auth)/forgot-password'}>
