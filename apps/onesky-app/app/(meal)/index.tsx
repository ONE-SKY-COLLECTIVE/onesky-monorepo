  ...
  const trackYourMealInformation = [
    {
      id: 'thecloud',
      icon: icons.thecloud,
      description: 'Help to reduce carbon footprint by lowering your meat intake.',
    },
    {
      id: 'star',
      icon: icons.star,
      description: 'Log your meal 3x a day and earn maximum points.',
    },
    {
      id: 'planet',
      icon: icons.planet,
      description: 'Join the One Sky community in making a difference for our environment.',
    },
  ];
  ...
  const mealTypes = [
    {
      id: 'vegan',
      title: 'Vegan meal:',
      icon: icons.veganMeal,
      description:
        'You’re a star! Switching to a vegan diet slashes greenhouse gas and land use by up to 75%, also uses 54% less water than meat-based meals.',
    },
    {
      id: 'vegetarian',
      title: 'Vegetarian meal:',
      icon: icons.vegetarianMeal,
      description:
        'Way to go! Vegetarian diets produce about 50% fewer greenhouse gases, require 27% less energy, and leave a 41.5% smaller environmental footprint compared to meat-based meals',
    },
    {
      id: 'flexitarian',
      title: 'Flexitarian meal:',
      icon: icons.flexitarianMeal,
      description:
        'Great work! By swapping out your red meat, you have helped reduce our reliance on cattle farming which releases 74.5 million tons of methane, which is 30x more potent than CO2, each year.',
    },
  ];
      ...
      <SafeAreaView className="h-full w-full relative" edges={['top', 'bottom']}>
        {!remindMeMealLog && !remindMe ? (
          ...
          <View className="w-full">
            <ProgressBar
              progression={totalMeals}
              numProgressions={3}
              points={50}
              utility={handleExitWithoutMealLog}
            />
          </View>
        )}
        {!remindMeMealLog && !remindMe && (
          ...
          <>
            ...
            <View className="flex-1 flex-col gap-4 m-4">
              {mealTypes.map(mealType => (
                ...
                >
                  <View
                    className={`meals-radio ${selectedMealId !== mealType.id ? 'border border-[#E7E5E4]' : 'border-[2px] border-[#A1CE3F]'}`}
                  />
        ...
        {confirm && (remindMeMealLog || remindMe) && (
          <>
            <Image
              resizeMode="contain"
              source={selectedMealId ? mealImages[selectedMealId] : undefined}
              className="w-3/5 mx-auto"
            />
            <View className="mx-auto w-10/12 -mt-12">
              <Text className="text-center text-4xl">
                {selectedMealId
                  ? mealTypes.find(meal => meal.id === selectedMealId)?.title
                  : undefined}
              </Text>
            </View>
            <View className="mx-auto w-10/12 mt-4">
              <Text className="text-center text-xl">
                {selectedMealId
                  ? mealTypes.find(meal => meal.id === selectedMealId)?.description
                  : undefined}
              </Text>
            </View>
            <View className={`${confirm ? 'refill-div-confirm' : ''} refill-div `}>
              <View className="yellow-bg-500 rounded-[100px] p-3 text-[12px] fit-width self-start">
                <Text>
                  +{totalMeals} {totalMeals === 1 ? 'log added today' : 'logs added today'}
                </Text>
              </View>
              ...
              <Text className="text-[14px] raleway">
                {totalMeals >= dailyGoal
                  ...
                  : 'Keep sharing your meal log'}
              </Text>
              {confirm && (
                <Image
                  resizeMode="contain"
                  source={icons.mealPlanet}
                  className="absolute size-[100px] left-3/4 top-[20px]"
                />
              )}
              {!confirm ? (
                <TouchableOpacity className={'w-full py-3 rounded-[8px] green-bg-500'}>
                  <Text className="text-center">Confirm meal log</Text>
                </TouchableOpacity>
              ) : (
                <View>
                  {totalMeals >= dailyGoal ? (
...
>>>>>>> refs/remotes/origin/develop
                    <View>
                      ...
                      <TouchableOpacity
                        onPress={() => handleAdditionalSubmit()}
                        className="green-bg-500 w-full py-3 rounded-[8px] mt-2"
                      >
                        <Text className="text-center">Submit another meal log</Text>
                      </TouchableOpacity>
                    </View>
                  )}
                </View>
        ...
        {exitWithoutMealLog && (
          <View className="refill-div">
            ...
            <Text className="text-[14px] raleway w-half">
              Don’t leave us yet without logging your meal.
            </Text>
            <Image
              resizeMode="contain"
              source={icons.leave}
            ...
            <View>
              ...
              <TouchableOpacity
                onPress={handleContinueMealLog}
                className="green-bg-500 w-full py-3 rounded-[8px] mt-2"
              >
                <Text className="text-center">Continue logging</Text>
              </TouchableOpacity>
            </View>
          </View>
        )}
