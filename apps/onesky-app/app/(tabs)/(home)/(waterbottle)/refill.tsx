  ...
  if (acknowledged) {
    return (
      <SafeAreaView className="h-[100vh]" edges={['bottom']}>
        ...
        <Image
          contentFit="contain"
          source={images.refillImage}
          style={styles.refillImage}
          alt="picture of waterbottle"
        ></Image>
        ...
        <View className="flex-col w-full px-4">
          <View className="flex items-center my-3">
            <Image
              contentFit="contain"
              source={icons.refillBottle}
              style={styles.icon}
              alt="bottle"
            ></Image>
            <Text className="text-[13px] flex-shrink w-0 flex-grow">
              Help reduce plastic waste by using your reusable water bottles
            </Text>
          </View>
          <View className="flex items-center my-3">
            <Image
              contentFit="contain"
              source={icons.refillStar}
              style={styles.icon}
              alt="star"
            ></Image>
            <Text className="text-[13px] flex-shrink w-0 flex-grow">
              Complete 7 refills daily to earn maximum points
            </Text>
          </View>
          <View className="flex items-center my-3">
            <Image
              contentFit="contain"
              source={icons.refillGroup}
              style={styles.icon}
              alt="group"
            ></Image>
            <Text className="text-[13px] flex-shrink w-0 flex-grow">
              Join the One Sky community in making a difference for our environment
            </Text>
          </View>
        </View>
        <View className="grow flex-col-reverse mb-12 px-4">
          ...
          <View className="flex">
            ...
            <Text className="text-[16px] mb-5 ml-3 text-[#171717] font-light">
      ...
      <SafeAreaView className="h-full" edges={['top', 'bottom']}>
        ...
        {!confirmation ? (
          <View className={`${confirm ? 'refill-div-confirm' : ''} refill-div `}>
            <View
              className={`${confirm ? 'bg-[#D8F5FA]' : 'bg-[#FFF6E5]'} rounded-[100px] p-3 text-[12px] fit-width self-start`}
            >
              <Text className={`${confirm ? 'text-[#3F5C61]' : 'text-[#D68F0E]'}`}>
                {totalRefills} bottles filled today
              </Text>
            </View>
            <View className="flex justify-between w-full">
              <View className="w-[60%]">
                ...
                {confirm && totalRefills < dailyGoal && (
                  <Text className="mt-5">
                    Can’t finish all? No worries—you’ll still get{' '}
                    <Text className="font-semibold">{dailyGoal - totalRefills}</Text> points when
                    you return tomorrow.
                  </Text>
                )}
              </View>
